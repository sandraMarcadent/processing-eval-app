...
from .models import Choice, Question, Voter512


class IndexView(generic.ListView):
    template_name = 'polls_512/index.html'
...
class DetailView(generic.DetailView):
    model = Question
    template_name = 'polls_512/detail.html'
...
class EndFormView(generic.DetailView):
    model = Question
    template_name = 'polls_512/end_form.html'
...
def vote(request, question_id):
    ...
    if Voter512.objects.filter(question_id=question.id, user_id=request.user.id).exists():
        
        return render(request, 'polls_512/detail.html', {
                'question': question,
    ...
    try:
        ...
        v = Voter512(user=request.user, question=question, choice_text = selected_choice.choice_text) 
        v.save()
        ...
        if question.is_last:
            return HttpResponseRedirect(reverse('polls_512:end_form', args=(question.id,)))
        else:
            return HttpResponseRedirect(reverse('polls_512:detail', args=(question.id+1,)))
...
def get_finalacc(request):
        ...
        for question in Question.objects.all():
            ...
            if Voter512.objects.filter(question_id=question.id, user_id=request.user.id).exists():
                ...
                v = Voter512.objects.get(question_id=question.id, user_id=request.user.id)
        ...
        return render(request, 'polls_512/fresults.html', {
